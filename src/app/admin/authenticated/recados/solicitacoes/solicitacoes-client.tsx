...
interface Solicitacao {
  ...
  recado?: { id: number; titulo: string } | null;
  unidadeId: number;
...
interface Props {
  ...
  userRole: "OWNER" | "ADMIN" | "MESSAGEONLY" | "NEWSONLY" | "MESSAGENEWS";
  userId: string;
...
export default function SolicitacoesClient({
  ...
  const revisar = async (id: number, acao: "aprovar" | "recusar") => {
    ...
    setProcessandoId(id);
    try {
      const res = await fetch(`/admin/api/recados/solicitacoes/${id}`, {
        method: "PUT",
  ...
  const cancelar = async (id: number) => {
    ...
    setProcessandoId(id);
    try {
      const res = await fetch(`/admin/api/recados/solicitacoes/${id}`, { method: "DELETE" });
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <h1 className="text-2xl font-bold tracking-tight text-gray-900 dark:text-white">
                {podeRevisar ? "Solicitações de Recados" : "Minhas Solicitações"}
              </h1>
          ...
          {listaFiltrada.map((s) => {
            ...
            return (
              ...
              >
                  ...
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-gray-400 flex items-center gap-1">
                        <MapPin className="w-3 h-3" /> {s.unidade?.nome} · por {s.solicitanteNome} ·{" "}
                ...
                {s.tipo === "DELETE" ? (
                  <p className="text-xs text-gray-500 dark:text-gray-400">
                    Solicita a exclusão do recado <strong>&ldquo;{s.recado?.titulo}&rdquo;</strong>.
                  </p>
                ) : (
