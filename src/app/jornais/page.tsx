    ...
    <div className="max-w-7xl mx-auto p-6 lg:p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {jornais.map((jornal) => (
          <article 
            key={jornal.id}
            className="group/card block bg-white/90 dark:bg-gray-900/90 backdrop-blur-xl rounded-3xl border border-gray-200/50 dark:border-white shadow-lg hover:shadow-2xl hover:border-blue-400/60 transition-all duration-500 hover:-translate-y-2 hover:scale-[1.015] h-[640px] relative overflow-hidden"
          >
            <div className="flex flex-col h-full p-6 transition-all duration-500">
              
              {/* Imagem EXPANDE! */}
              <div className="relative flex-shrink-0 overflow-hidden rounded-2xl mb-4 transition-all duration-700 group-hover/card:h-[22rem] h-[16rem]">
                <Image
                  src={`/uploads/jornais/${jornal.imagem}`}
                  alt={jornal.titulo}
                  fill
                  sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw"
                  className="object-cover transition-all duration-700 group-hover/card:scale-[1.08]"
                />
              ...
              <div className="flex-1 flex flex-col">
                {/* Título */}
                <h3 className="px-4 mb-4 text-lg md:text-xl font-bold text-gray-900 dark:text-white leading-tight line-clamp-2 group-hover/card:line-clamp-none group-hover/card:leading-relaxed transition-all duration-300">
                  {jornal.titulo}
                </h3>
                
                {/* Descrição - MAXIMA EXPANSÃO! */}
                <div className="flex-1 px-4 overflow-hidden transition-all duration-700 group-hover/card:max-h-64 max-h-[4.5rem] mb-6">
                  <p className="text-sm md:text-base text-gray-600 dark:text-gray-300 leading-relaxed group-hover/card:line-clamp-none line-clamp-3 hyphens-auto">
                    {jornal.descricao || "Clique para ler a edição completa do jornal da Naturafrig!"}
                  </p>
                </div>
                
                {/* Footer */}
                <div className="px-4 pt-4 border-t border-gray-100/50 dark:border-gray-800/50 mt-auto">
                  <div className="flex items-center justify-between">
                    ...
                    <a
                      ...
                      className="text-xs md:text-sm font-semibold text-blue-600 hover:text-blue-700 opacity-0 group-hover/card:opacity-100 transform translate-x-2 group-hover/card:translate-x-0 transition-all duration-300"
                    >
                      ↗️ Abrir
                    </a>
                  </div>
              ...
              <a 
                ...
                aria-label={`Abrir ${jornal.titulo}`}
              />
            </div>
          </article>
        ))}
